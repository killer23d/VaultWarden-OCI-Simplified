#!/usr/bin/env bash
# update.sh - Simplified VaultWarden container and system updates
# Replaces: Complex watchtower and maintenance systems
...
update_containers() {
    ...
    for service in $services; do
        local image new_id current_running_id
        image=$(docker compose config | grep -A 10 "$service:" | grep "image:" | awk '{print $2}' | head -1)
        new_id=$(docker images --format "table {{.Repository}}:{{.Tag}}	{{.ID}}" | grep "$image" | awk '{print $2}' | head -1 || echo "unknown")

        # Get currently running container image ID
        current_running_id=$(docker compose ps -q "$service" 2>/dev/null | xargs -r docker inspect --format '{{.Image}}' 2>/dev/null | head -1 || echo "")

        if [[ -n "$current_running_id" && "$new_id" != "${current_running_id:0:12}" ]]; then
